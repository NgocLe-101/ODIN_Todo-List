...
export default class UI {
    ...
    const renderTaskList = (taskList) => {
      ...
      if (taskList.length === 0) {
        ...
        taskList.forEach((task) => {
          ...
          taskParser.id = task.id;
          taskParser.addEventsToIcon({
            'click': () => {
              Storage.updateTaskInProject(Storage.getActiveProject().id, {
                id: task.id,
                state: taskParser.getTask().getState()
              });
              this.#_pubSub.publish('taskStateChange', task.id);
            }
          })
          const taskElem = taskParser.parse();
          taskElem.addEventListener('click', (e) => {
            const targetContainer = e.target.closest('div');
            if (!targetContainer.classList.contains('icon-wrapper')) {
  ...
  initPubSub() {
    ...
    this.#_pubSub.subscribe('taskStateChange', (taskID) => {
      // const taskList = document.querySelector('.main-content .top .task-list-container');
      ...
      if (task.state.dueDateExceeded) {
        taskElem.classList.add('due-date-exceeded');
      } else {
        taskElem.classList.remove('due-date-exceeded');
      }
    });
